...
URL = "http://10.82.136.170:5000/oracle"
...
def brute_forcer(reference_chunk, initial_text, block_size, offset, secret):
    ...
    for char in charlist:
        ...
        test_text = initial_text + secret + char
        print ('Testing test: ' + str(test_text))
...
def extract_secret(block_size, offset):
    ...
    while char is not None:
        ...
        #Now we can start the brute force
        char = brute_forcer(reference_chunk, initial_text, block_size, offset, secret)
        secret += char
        i += 1
    return secret
...
if __name__ == '__main__':
    ...
    print ("Brute forcing the secret")
    secret = extract_secret(size, offset)
